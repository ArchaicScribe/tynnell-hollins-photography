...
// Payload relationship field value: populated object, ID, or null
...
export function CoverPhotoPicker() {
  const { value: rawValue, setValue } = useField<RelValue>({ path: 'coverPhoto' })
  ...
  // Fetch when open state or filters change
  useEffect(() => {
    if (!open) return
  ...
  const selectPhoto = (photo: PhotoDoc) => {
    setValue(photo)
    setOpen(false)
  }
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
        ...
        >
          {current?.url ? (
            ...
            <img
              src={current.url}
              alt={current.alt ?? current.filename ?? 'Cover photo'}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
          <button type="button" onClick={openPicker} style={btnStyle}>
            {current ? 'Change Cover Photo' : 'Choose Cover Photo'}
          </button>
          {current?.filename && (
          ...
          {!current && (
            <span style={{ fontSize: '0.65rem', color: 'rgba(155,154,154,0.5)' }}>
              Required - shown on your portfolio page
            </span>
          )}
        </div>
      </div>
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0.85rem 1.25rem' }}>
              {loading ? (
                ...
                >
                  {photos.map(photo => {
                    const isCurrentCover = current?.id === photo.id
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          overflow: 'hidden',
                          cursor: 'pointer',
                          border: `2px solid ${isCurrentCover ? 'rgba(214,209,206,0.8)' : 'transparent'}`,
                          boxSizing: 'border-box',
                        }}
                      >
                        {src ? (
