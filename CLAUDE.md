...
| Layer | Technology |
|---|---|
| Framework | Next.js 16.x (pinned to an exact version in package.json, currently 16.3.1), App Router, TypeScript |
...
- **Next 16: `next lint` no longer exists.** `npm run lint` is now `eslint .` (still driven by the existing `.eslintrc.json`). **The request gate lives in `proxy.ts`, not `middleware.ts`** (renamed for Next 16 with `npx @next/codemod@canary middleware-to-proxy`; the only change is the exported function name, `middleware` to `proxy`). Next errors at build time if both files exist. The Edge Runtime deprecation is also resolved: `app/(site)/opengraph-image.tsx` no longer declares `runtime = 'edge'`, so it prerenders statically like everything else. The build now emits no deprecation warnings at all. That route reads `SiteDesign` and `SiteConfig` for its palette and business name rather than hardcoding them, which was only possible once it left the edge runtime (those helpers pull in payload).
- **The Next version is constrained by `@payloadcms/next`, not by preference.** Its peer range is narrow and non-obvious (`>=15.2.9 <15.3.0 || >=15.3.9 <15.4.0 || >=15.4.11 <15.5.0 || >=16.2.6 <17.0.0`). The project ran 15.5.19 for a long time, which satisfies NONE of those windows; `legacy-peer-deps=true` in `.npmrc` silently suppressed the error. Check the peer range before changing the Next version, and remember npm will not warn you.
- **`vitest` excludes `.claude/**`.** Worktrees under `.claude/worktrees` are full checkouts of other branches, and vitest was collecting their test files too, reporting failures from stale copies of code not in this tree. Real suite: 18 files, 167 tests.
- **Fonts are self-hosted, do not add `next/font/google` back.** `next/font/google` fetches every face from `fonts.gstatic.com` at BUILD time. With nine layouts pulling nine families, one throttled response fails the whole build, which is exactly what killed a qa deploy on 2026-08-10 when four branches built at once. All faces now load via `next/font/local` from `app/fonts/google` (basic-latin woff2 only, ~840 KB) through one shared `app/fonts/fonts.ts`. Regenerate with `node scripts/fetch-google-fonts.mjs` if a weight is added. **`next/font` requires explicitly written literals**: a `w(file, weight)` helper or a shared `const POPPINS_ALL = [...]` array both fail the build with "Font loader values must be explicitly written literals", which is why every `src` entry is longhand. Do not refactor them. **The Payload admin is self-hosted too**: `app/(payload)/custom.css` declares its own `@font-face` rules pointing at `public/fonts/admin` (Archivo + Roboto Mono only; the old `@import` also pulled Tangerine, which that stylesheet never used). Those live in `public/` rather than going through `next/font` because Payload's admin CSS needs a stable URL and `next/font` emits hashed paths. **The CSP therefore has no Google origins at all** (`style-src 'self' 'unsafe-inline'`, `font-src 'self'`). If a font stops rendering after a CSP change, something reintroduced a remote font rather than the policy being too strict.
