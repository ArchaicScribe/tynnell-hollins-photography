...
- **Next 16: `next lint` no longer exists.** `npm run lint` is now `eslint .` (still driven by the existing `.eslintrc.json`). Two deprecation warnings are expected on every build and are NOT yet actioned: the `middleware` file convention is deprecated in favour of `proxy` (verified working on 16 regardless: the COMING_SOON gate and the `/admin` to `/studio` redirect both behave correctly), and the Edge Runtime is deprecated, used only by `app/(site)/opengraph-image.tsx`, which also costs that route static generation. Both are follow-ups, not blockers.
